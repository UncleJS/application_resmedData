...
// threshold (1500) so zooming still re-windows against real detail.
const MAX_PLD_POINTS = 4000;
...
export default async function SessionDetailPage({
  ...
  const [session, pldFull, events, brp1s] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="font-mono text-sm text-foreground">{formatTs(session.session_start_utc)}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-sm text-foreground">
        ...
        <span>PLD samples: <span className="text-foreground">{pldFull.length.toLocaleString()}</span></span>
