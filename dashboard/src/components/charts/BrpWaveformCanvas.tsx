...
import type { BrpFullRow } from "@/lib/queries/brp";
...
function calcBucket(durationMs: number, canvasWidth: number): number {
  const targetPoints = canvasWidth * 1.5; // ~1.5 points per pixel
...
function toEpochMs(sessionStartIso: string, offsetMs: number): number {
  return new Date(sessionStartIso).getTime() + offsetMs;
}

export default function BrpWaveformCanvas({ sessionId, sessionStart, height = 320 }: Props) {
  ...
  // View window: [viewStart, viewEnd] in ms offset
  ...
  const fetchData = useCallback(
    async (start: number, end: number, width: number) => {
      setLoading(true);
      setError(null);
      const bucket = calcBucket(end - start, width);
      try {
        const res = await fetch(`/api/brp/${sessionId}?bucket=${bucket}`);
  ...
  // Initial load: fetch full session at coarse resolution
  useEffect(() => {
  ...
  // After initial load, set view window to full extent
  useEffect(() => {
    if (data.length === 0) return;
    const maxMs = data[data.length - 1]!.offset_ms;
    if (totalDurationMs.current === 0) {
  ...
  useEffect(() => {
    ...
    const visible = data.filter(
      (d) => d.offset_ms >= viewStart && d.offset_ms <= viewEnd
    );
    ...
    for (let i = 0; i <= tickCount; i++) {
      ...
      ctx.fillText(fmtHHMMSS(toEpochMs(sessionStart, ms)), x, H - 6);
    }
    ...
    if (flows.length > 0) {
      ...
      if (lastVisible) ctx.lineTo(tX(lastVisible.offset_ms), PAD.top + plotH);
      ctx.closePath();
    ...
    if (flows.length > 0) {
      ...
      for (const d of visible) {
        if (d.flow_l_s == null) continue;
        const x = tX(d.offset_ms); const y = yFlow(d.flow_l_s);
    ...
    if (pressures.length > 0) {
      ...
      for (const d of visible) {
        ...
        const x = tX(d.offset_ms); const y = yPress(d.pressure_cmh2o);
        first ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        first = false;
      }
  ...
  const handleWheel = useCallback(
    (e: WheelEvent) => {
      ...
      const minSpan = 5_000; // 5s minimum view
      ns = Math.max(0, ns);
      ne = Math.min(totalDurationMs.current, ne);
      if (ne - ns < minSpan) { ns = Math.max(0, pivot - minSpan / 2); ne = ns + minSpan; }
      setViewStart(ns);
      setViewEnd(ne);
    },
    [viewStart, viewEnd]
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const ms = viewStart + (px / plotW) * (viewEnd - viewStart);
    setHoverMs(ms);
    ...
    if (drag.current) {
      ...
      if (ns < 0) { ne -= ns; ns = 0; }
      if (ne > totalDurationMs.current) { ns -= (ne - totalDurationMs.current); ne = totalDurationMs.current; }
      setViewStart(Math.max(0, ns));
      setViewEnd(Math.min(totalDurationMs.current, ne));
    }
  }, [viewStart, viewEnd]);
  ...
  const resetZoom = useCallback(() => {
    setViewStart(0);
    setViewEnd(totalDurationMs.current);
  }, []);
  ...
  // ── zoom info ───────────────────────────────────────────────────────────────
  const zoomRatio = totalDurationMs.current > 0
    ? Math.round((totalDurationMs.current / Math.max(1, viewEnd - viewStart)) * 10) / 10
    : 1;
  ...
  return (
    <div ref={containerRef} className="w-full select-none">
      ...
      <div className="flex items-center gap-3 mb-2 text-xs text-muted-foreground">
        ...
        {hoverMs !== null && (
          <span className="font-mono">cursor: {fmtHHMMSS(toEpochMs(sessionStart, hoverMs))}</span>
        )}
        <button
          onClick={resetZoom}
